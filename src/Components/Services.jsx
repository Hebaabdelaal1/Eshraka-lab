...
<div className="bg-gray-50 py-16 px-6 md:px-20" id="services">
  ...
  >
   ...
   {/* ===== Devices Grid ===== */}
<div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto mb-20">
  ...
  >
    <div className="max-w-5xl mx-auto text-center">
      <h2 className="text-4xl font-extrabold mb-10 text-sky-700">
        Eshraka Services & List Price
      </h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {servicesList.map((service, idx) => (
          <div key={idx} className="bg-white shadow-lg rounded-xl p-6 flex flex-col items-center">
            <h3 className="text-lg font-bold text-gray-800 mb-4">{service.name}</h3>
            <span className="px-4 py-2 rounded-full bg-sky-100 text-sky-700 font-semibold text-lg shadow-sm">
              {service.price}
            </span>
          </div>
        ))}
